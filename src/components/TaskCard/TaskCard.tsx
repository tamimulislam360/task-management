...
const TaskCard = observer(({ task }) => {
  ...
  const updateByStatus = (e) => {
    setCurrentTask({ ...currentTask, status: e.target.value })
    updateTask(currentTask)
  }

  const color = task.status === 'To Do' ? 'text-pink-600' : task.status === 'In Progress' ? 'text-yellow-700' : 'text-green-600'
    
  return (
    <div className="group p-2 border bg-white shadow-sm relative hover:border-blue-600">
       
            <FiExternalLink onClick={() => detailsHandleOpen(task)} className="w-6 h-6 text-white rounded-full absolute top-2 right-2 hidden group-hover:block bg-blue-600 cursor-pointer p-1" />
      ...
      <div className="flex gap-1 justify-between items-center mt-2">
            <select className={`cursor-pointer outline-none focus:outline-none pr-2 text-sm ${color} border-none`} value={task.status} onChange={(e) => updateByStatus(e)} >
                <option className="border-none" value="To Do">To Do</option>
                <option className="border-none" value="In Progress">In Progress</option>
                <option className="border-none" value="Completed">Completed</option>
            </select>
        <div className="flex gap-1 items-center justify-end">
          ...
          <AiOutlineDelete onClick={() => deleteTask(task.id)} className="cursor-pointer " />
        </div>
      </div>
      {editing && (
        <Modal
